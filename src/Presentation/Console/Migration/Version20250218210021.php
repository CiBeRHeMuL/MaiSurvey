 ...
 */
final class Version20250218210021 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql(
            <<<SQL
CREATE MATERIALIZED VIEW my_survey(id, user_id, completed, completed_at, subject_id, actual) AS
...
SQL,
        ...
        $this->addSql(
            <<<SQL
CREATE MATERIALIZED VIEW my_survey_item(id, survey_id, user_id, teacher_subject_id, student_subject_id, position) AS
SELECT si.id,
       ms.id AS survey_id,
       ms.user_id,
...
SQL,
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql(
            <<<SQL
CREATE VIEW my_survey(id, user_id, completed, completed_at, subject_id, actual) AS
SELECT DISTINCT ON (s.id, u.id) s.id,
                                u.id                AS user_id,
                                cs.* IS NOT NULL    AS completed,
                                cs.created_at       AS completed_at,
